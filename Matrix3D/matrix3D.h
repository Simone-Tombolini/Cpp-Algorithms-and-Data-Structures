...
/**
 * @brief Funtore di default
 * Funtore di default per l'operatore ==, questo funtore riutilizza l'operatore per provare due uguaglianze
 * fra due tipi T uguali, nel caso non fosse definito l'operatore per un tipo
 * custom il compilatore darebbe errore, non trovando un funtore adeguato.
 * Rimane comunque possibile definire un operatore differente nel caso in cui non fosse 
 * disponibile l'operatore ==.  
 * @tparam T Tipo di confronto del parametro
 */
template<typename T>
struct eql_default {
...
/**
 * @brief Classe che rappresenta una matrice 3D
 * Classe principale del file, rappresenta una matrice in 3 dimensioni.
 * Per le specifiche complete fare riferimento al testo d'esame e alla relazione. 
 * @tparam T Parametro templato che definisce il tipo di dati della matrice
...
public:
	 ...
	 * Costruisce una matrice vuota, dove il puntatore è a nullptr e le
	 ...
	 * Costruisce una matrice dove le dimensioni sono impostate ai 3 parametri di ingresso,
		...
		_matrix(nullptr), sz_z(0), sz_y(0), sz_x(0), _dim(0)
	{
		...
		// se anche solo una size e' a 0, ma non tutte sono a 0 lancio l'eccezzione, perchè ho fatto il contorllo prima
			...
			try {
				...
				//togliere commento riga 102 solo se si sta testando il fallimento della new, ricordarsi di aggiungere ;  
				//throw;
		...
		_matrix(nullptr), sz_z(0), sz_y(0), sz_x(0), _dim(0) // in caso di fallimento rimangono in uno stato coerente
	{
		if ((size_x != 0) && (size_y != 0) && (size_z != 0)) {//se tutte sono a 0 allora è come se richiamasse un costruttore vuoto
			...
			try {
				...
				//togliere commento riga 151 solo se si sta testando il fallimento della new, ricordarsi di aggiungere ; 
				//throw
	...
	/**
	 * @brief Distruttore
	 *	Richiama il metodo clear() per eliminare la matirce anche dallo heap ed eviatare tutti i memory leak
		...
		_matrix(nullptr), sz_z(0), sz_y(0), sz_x(0), _dim(0)
	{
		...
		try {
			...
			//togliere commento riga 205 solo se si sta testando il fallimento della new, ricordarsi di aggiungere ; 
			//throw
		...
		//in questo caso la classe potrebbe essere in uno stato incoerente, e quindi effettuo una clear() 
		//per riportarla ad uno stato coerente e rilancio l'eccezione
		//questo blocco verra' ripetuto piu' volte nel progetto, questi commenti sono da considerarsi vaidi 
		catch (...)
		{
			clear();//evito memory leak
			throw;//rilancio l'eccezzione
	 ...
	 * Il tipo di partenza della matrice è il tipo C, quello di arrivo è quello definito
	 * dalla nuova matrice che iol costruttore sta creando.
	 ...
	 */
	template <typename C, typename E2 = eql_default<C>>
	matrix3D(const matrix3D<C, E2>& other)
		: _matrix(nullptr), sz_z(0), sz_y(0), sz_x(0), _dim(0)
	{
		try {
			...
			//togliere commento riga 253 solo se si sta testando il fallimento della new, ricordarsi di aggiungere ; 
			//throw
	...
	T& operator[](const size_type i)
	{
	 ...
	 /**
	 * @brief Operatore ad una dimensione che ritorna il dato costante contenuto in una cella della matrice 
	 * Ritorna il parametro della matrice ad una posizione monodimensionale, l'ordine è lo stesso degli
	 * iteratori, rendendo questo metodo un alternativa alla creazione di un iteratore ed all'utilizzao del metodo[]
	 * dell'iteratore.
	 ...
	 * l'operatore ritorna true, altrimenti ritorna false.
	 ...
	 * essendo che la maggior parte delle volte quell'implementazione è corretta ho ritenuto opportuno implementare
	 * un metodo veloce di utilizzo dei funtori.
	 ...
	 * La logica di confronto è definita da un funtore generico di tipo E.
	 * Nel caso il funtore dia un ritorno positivo per tutti i valori e le dimensioni siano uguali
	 * l'operatore ritorna false, altrimenti ritorna true.
	 * E' definito un funtore di default all'intenro del file della classe, che utilizza a sua volta
	 * l'operatore == del tipo T, che deve essere definito per poter accedere a questa possibilita',
	 * essendo che la maggior parte delle volte quell'implementazione è corretta ho ritenuto opportuno implementare
	 * un metodo veloce di utilizzo dei funtori.
...
//cambio
	 ...
	 * @param set se impostato a false nullifica la funzione
	 */
	void print_message(const char *message, const bool set = true) {
		#ifndef NDEBUG
		if (set == true) 
		{
			cout << message << endl;
	 ...
	 * @param set se impostato a false nullifica la funzione
	 */
    void print_matrix(const char *message = "", const bool set = true) const
    {
 ...
 * @return matrix3D<Q> 
 */
template<typename T, typename Q, typename F, typename E = eql_default<T> >
matrix3D<Q> transform(const matrix3D<T, E> &matrix) 
{
	
	F f;
	matrix3D<Q> res_matrix(matrix.size_z(), matrix.size_y(), matrix.size_x());
	/*typename matrix3D<T>::iterator b, e;
